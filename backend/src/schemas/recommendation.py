...
class RecommendationCreate(RecommendationBase):
    ...
    recommendation_type: str = Field(
        default="ai_match",
        description="Type of recommendation: ai_match, user_offer, similar"
    )
    match_score: float = Field(default=0.0, ge=0, le=100, description="Match confidence score (0-100)")
    match_reasons: Optional[list[str]] = Field(default=None, description="Reasons for the match")
    message: Optional[str] = Field(default=None, max_length=1000, description="Optional message from user")
